...
describe Quacks do
  ...
  it "should retrieve all previously saved quotes" do
    ...
    quotes = subject.quotes_for_author(name, db)
  ...
  it "should retrieve all quotes" do
    ...
    name3 = "n3"
    quote3   = "q3"
    subject.save_quote(name1, quote1, db)
    subject.save_quote(name2, quote2, db)
    subject.save_quote(name3, quote3, db)
    subject.all_quotes(db).length.must_equal 3
    subject.all_quotes(db).first.must_match /^q[123]/
  end
  ...
  it "change an image file path for an author" do
    ...
    image_path = "path"
    subject.save_image(name, old_image_path, db)
    subject.image_path_of_author(name, db).must_equal old_image_path
    subject.save_image(name, image_path, db)
    subject.image_path_of_author(name, db).must_equal image_path
  end
end
